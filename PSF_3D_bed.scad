...
module bed(){
	...
	difference (){
		union(){
			...
			translate([y_belt_center,size_y/2-7,42+4]) cube([20,50,10],center = true);
			
			// slide unit support
			translate([y_load_offset_x,size_y/2,y_load_offset_z+7]) cube([20,90,20],center=true);
			translate([size_x-y_load_offset_x,size_y/2,y_load_offset_z+7]) cube([20,90,20],center=true);
		}
		...
		// cable tie hole
		tie_hole(y_load_offset_x,size_y/2,y_load_offset_z);
		tie_hole(y_load_offset_x,size_y/2-40,y_load_offset_z);
		tie_hole(y_load_offset_x,size_y/2+40,y_load_offset_z);
		
		tie_hole(size_x-y_load_offset_x,size_y/2,y_load_offset_z);
...
module bed_parts()
{
	...
	translate([y_load_offset_x,size_y/2,y_load_offset_z]) rotate([90,0,0]) lm8uu(90);
	translate([size_x-y_load_offset_x,size_y/2,y_load_offset_z]) rotate([90,0,0]) lm8uu(90);
}
