...
module test_belt() {
	...
	translate([0,0,0])color("aquamarine") {
		belt_len(tGT2_2,5, 50);
		translate([50,30,0]) rotate([0,0,180]) belt_len(tT2_5,5,50);
		translate([ 0,30,0]) rotate([0,0,180]) belt_angle(tT2_5,6,5,180);
		translate([50,0,0]) rotate([0,0,0]) belt_angle(tT2_5,6,5,180);
	}
}
...
module _belt_len(prf = -1, len = 10, bwdth = 5) {
	...
	translate ([0,0,-bwdth/2]) intersection() {
		union(){
			for( i = [0:n]) {
				//draw_tooth(prf,i,bwdth); //+++ 속도를 위해서 주석 처리
			}
